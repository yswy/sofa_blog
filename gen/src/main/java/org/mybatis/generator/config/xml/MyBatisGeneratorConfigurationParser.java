...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        parseTableFormDalConfig(context);
    }
    ...
    protected void parseTableFormDalConfig(Context context) {

        try {
            File dalConfig=new File(context.getProperty("targetRunPath"),"dal-config.xml");
            if(!dalConfig.exists()){
                return;
            }
            DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
            DocumentBuilder builder = factory.newDocumentBuilder();
            Document document = builder.parse(new InputSource( new FileReader(dalConfig)));
            NodeList nodelList=document.getElementsByTagName("include");
            for(int i=0;i<nodelList.getLength();i++){
                TableConfiguration tc = new TableConfiguration(context);
                tc.setTableName(nodelList.item(i).getAttributes().getNamedItem("table").getNodeValue());
                context.addTableConfiguration(tc);
            }

        } catch (Exception e) {
            throw new RuntimeException("构造自定义dalConfig报错",e);
        }
